/**
 * blog http://www.cnblogs.com/hustskyking/p/websocket-with-node.html
...
net.createServer((o) => {
  ...
  o.on('data', (data) => {
    if (!key) {
      ...
      let close
      if ( frame.Opcode == 8) {
        // 判断 是否服务器主动关闭（这时 o.end 报错）
        o.end(); //断开连接
        console.warn('断开连接');
      }else{
        console.log('o.Socket:',o.Socket);
        if(close === undefined){
          // 服务器主动关闭
          close = setTimeout(()=>{
            console.log('send Opcode 8');
            o.write(encodeDataFrame({
              FIN: 1,
              Opcode: 8,
              PayloadData: 'close-yyj'
            }));
          },3000)
        }
      }
